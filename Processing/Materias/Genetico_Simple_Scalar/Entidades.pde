...
float correrSimulacion() {
  ...
  try {
    //Runtime.getRuntime().exec("chmod 777 confs", null, workingDir);
    Process p = Runtime.getRuntime().exec(commandToRun, null, workingDir);
    
    // leer la ultima fila de un archivo de texto
    String[] lines; int index;
    lines = loadStrings(workingDir + "/results/res.txt");
    index = lines.length;
    println("ind: " + index);
    println(lines[index-1]);
    float IPC = float(lines[index-1]);
    println(IPC);
    if(IPC != 0){
      return IPC+offset;
    }
    //String[] commandToRun = {"sh", workingDir+"/sc_simooo.sh"};
    //String[] commandToRun = {"bash", "-c", "/home/piaktipik/Desktop/SoCs/Practica2/Lab/sc_simooo.sh"};
