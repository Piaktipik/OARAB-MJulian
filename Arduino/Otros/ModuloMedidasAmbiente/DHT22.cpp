...
DHT22_ERROR_t DHT22::readData()
{
  ...
  // se verifica tiempo entre llamados
  ...
  // se verifica que el pin este en alto 
  ...
  // si el pin esta en alto se envia pulso de activacion
  // Send the activate pulse
  cli();
  DIRECT_WRITE_LOW(reg, bitmask);
  DIRECT_MODE_OUTPUT(reg, bitmask); // Output Low
  sei();
  delayMicroseconds(900); // 1.1 ms
  cli();
  DIRECT_MODE_INPUT(reg, bitmask);	// Switch back to input so pin can float
  sei();

  // medimos cuanto tiempo pasa hasta recivir la respues del sensor
  ...
  // vericiamos la señal de respuesta, el pin deve estar en alto 80us
  // Find the end of the ACK Pulse
  retryCount = 0;
  do
  {
    if (retryCount > 42) //(Spec is 80 us, 50*2 == 100 us)
    {
  ...
  // leemos los  datos donde: 0(50us low + 26us high) y 1(50us low + 70us high)
  ...
  for(i = 0; i < DHT22_DATA_BIT_COUNT; i++)
  {
    ...
    do
    {
      if (retryCount > 80) //(Spec is 80 us, 50*2 == 100 us)
      {
    ...
    // guardamos la demora del ultimo pico, con esto sabes si es 1 o 0
  ...
  //for(i = 0; i < DHT22_DATA_BIT_COUNT; i++)
  //{Serial.print(bitTimes[i]);if(i%8 == 0 ){Serial.print('-');}else{Serial.print(',');}}
  //Serial.println();
  ...
  // se extrae bits paridad (validacion)
  for(i = 0; i < 8; i++)
  {
    if(bitTimes[i + 33] > 25)
    {
